...
def capture_calibration_images():
    ...
    while True:
        ...
        frame_display = frame.copy()  # Make a copy to avoid modifying the frame for saving
        ...
        # Draw corners and other information for visualization (but not on saved image)
        if ret_chess:
            # Draw and display the corners (on the frame)
            cv2.drawChessboardCorners(frame_display, CHESSBOARD_SIZE, corners, ret_chess)
            cv2.putText(frame_display, "Chessboard detected!", (50, 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        
        # Display capture counter on the frame
        cv2.putText(frame_display, f"Captured: {img_counter}", (50, height - 50), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        ...
        # 'c' to capture
            ...
            # Save the image (without overlays)
            img_name = os.path.join(OUTPUT_DIRECTORY, f"calibration_{img_counter:02d}.jpg")
            cv2.imwrite(img_name, frame)  # Save the raw frame, no overlays
